...
function updateSelectionUI(){
  ...
  const available=bankOrder.filter(b=>selectedBanks.has(b)).reduce((n,b)=>n+questionBanks[b].length,0);
...
function allocateBalanced(total,banks){
  ...
  const picked=[];let remaining=total;let active=[...banks];
  while(remaining>0&&active.length){
    const share=Math.max(1,Math.floor(remaining/active.length));const next=[];
    for(const b of active){
      const take=Math.min(share,pools[b].length);if(take){picked.push(...pools[b].splice(0,take));remaining-=take;}
      if(pools[b].length)next.push(b);if(remaining<=0)break;
    }
    active=next;
  }
...
function buildPool(){
  ...
  const available=banks.reduce((n,b)=>n+questionBanks[b].length,0),countValue=$('questionCount').value;
  let questions=countValue==='all'?banks.flatMap(b=>questionBanks[b]):allocateBalanced(Math.min(Number(countValue),available),banks);
